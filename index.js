...
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
client.connect(err => {
  const clientOrderCollection = client.db("Solarstudio").collection("clientOrder");
  const clientReviewCollection = client.db("Solarstudio").collection("clientReview");
  const adminAddServiceCollection = client.db("Solarstudio").collection("addService");
  const makeAdminCollection = client.db("Solarstudio").collection("addAdmin");
  ...
  app.get('/getAllClientReview', (req, res) => {
    clientReviewCollection.find({}).limit(8)
    .toArray((error, documents) => {
        res.send(documents);
    })
  });
  ...
  app.get('/getAddedService', (req, res) => {
    adminAddServiceCollection.find({}).limit(8)
    .toArray((error, documents) => {
        res.send(documents);
    })
  });
